...
class ConversionThread(QThread):
    ...
    def clean_srt_text(self, text):
        """Metin içindeki italik hariç TÜM etiketleri (bold, color, size, pos) siler."""
        # 1. İtalikleri korumaya al (Büyük/Küçük harf duyarlı)
        text = re.sub(r'<(i|I)>', '[[i]]', text)
        text = re.sub(r'</(i|I)>', '[[/i]]', text)
        
        # 2. Tüm HTML etiketlerini temizle (<b...>, <font...>, <u...>, vb.)
        text = re.sub(r'<[^>]*>', '', text)
        
        # 3. Süslü parantez içindeki tüm stil kodlarını temizle ({b1}, {c&H...}, {pos...})
        text = re.sub(r'\{[^\}]*\}', '', text)
    ...
    def process_file_cleaning(self, file_path):
        """Dosyayı okur, temizler ve tekrar kaydeder."""
        try:
    ...
    def run(self):
        ...
        # 1. Analiz: Ses ve Dahili Altyazıları Tespit Et
        ...
        # 2. Dahili Altyazıları Dışarı Çıkar ve TEMİZLE
        cleaned_files = []
        ...
        # 3. Harici Altyazıları Bul ve TEMİZLE
        ...
        # 4. Final Muxing: Sadece Video, Ses ve Temizlenmiş Altyazıları Birleştir
...
if __name__ == "__main__":
    app = QApplication(sys.argv); app.setStyle("macos"); w = MainWindow(); w.show(); sys.exit(app.exec())        cleaned_internal_paths = []
        for i, sub in enumerate(internal_subs):
            temp_sub = os.path.join(temp_dir, f"int_{i}.srt")
