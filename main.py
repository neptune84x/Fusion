...
class ConversionThread(QThread):
    ...
    def process_ass_to_srt_with_italics(self, ass_path, srt_output_path):
        ...
        try:
            ...
            subprocess.run(
                [ffmpeg, '-y', '-i', ass_path, srt_output_path],
                capture_output=True
            )
    ...
    def run(self):
        ...
        if self.output_format == "mp4_vtt":
            ...
            chapter_meta = os.path.join(temp_dir, "chapters.txt")
            ...
            box_cmd.extend([
                "-add", f"{temp_mp4}#video",
                "-add", f"{temp_mp4}#audio"
            ])

            for i, c in enumerate(cleaned_list):

                disable = ":disabled" if i > 0 else ""

                box_cmd.extend([
                    "-add",
                    f"{c['path']}:lang={c['lang']}:group=2{disable}"
                ])

            if os.path.exists(chapter_meta):
                box_cmd.extend(["-chap", chapter_meta])
