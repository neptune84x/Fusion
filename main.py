...
class ConversionThread(QThread):
    ...
    def clean_subtitle_text(self, text):
        """
        Nihai İtalik Koruma Sistemi: Maskeleme yöntemiyle karakterleri zırh altına alır.
        """
        # 1. TÜM İtalik varyasyonlarını (ASS ve SRT) maskele
        # {\i1}, \i1, \i ve <i> etiketlerini güvenli bir bölgeye taşı
        ...
        # 3. Kalan TÜM süslü parantezli stil/renk kodlarını temizle ({...})
        text = re.sub(r'\{[^\}]*\}', '', text)
        
        # 4. Diğer tüm HTML benzeri etiketleri (font, u, s vb.) temizle
        text = re.sub(r'<[^>]*>', '', text)
        
        # 5. Maskelenmiş italikleri standart SRT <i> etiketine geri döndür
    ...
    def run(self):
        ...
        for i, sub in enumerate(internal_subs):
            ...
            # İtalik etiketlerini koruyarak SRT'ye aktarmaya çalış
        ...
        # MUXING: Chapters ve Metadata Koruma Devrede
...
# --- GUI BİLEŞENLERİ (Kesinlikle Korundu) ---
...
class SublerListWidget(QWidget):
    ...
    def show_context_menu(self, pos):
        menu = QMenu(self)
        # Sağ tık menüsündeki eksik seçenek geri eklendi
...
if __name__ == "__main__":
    app = QApplication(sys.argv); app.setStyle("macos"); w = MainWindow(); w.show(); sys.exit(app.exec())        text = text.replace('[[F_ITA_S]]', '<i>').replace('[[F_ITA_E]]', '</i>')
