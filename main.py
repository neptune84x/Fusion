...
class ConversionThread(QThread):
    ...
    def extract_chapters_ogm(self, input_file, output_path):
        """Bölüm isimlerini koruyarak OGM formatında dışarı aktarır."""
        ffmpeg = self.get_bin('ffmpeg')
        cmd = [ffmpeg, '-y', '-i', input_file, '-f', 'ffmetadata', 'temp_meta.txt']
        subprocess.run(cmd, capture_output=True)
        
        chapters = []
        if os.path.exists('temp_meta.txt'):
            with open('temp_meta.txt', 'r', encoding='utf-8') as f:
                content = f.read()
            
            # Simple OGM chapter generation logic
            matches = re.findall(r'\[CHAPTER\]\s+TIMEBASE=1/(\d+)\s+START=(\d+)\s+.*?title=(.*)', content)
            for i, (tb, start, title) in enumerate(matches, 1):
                ms = int(start) * (1000 / int(tb))
                h = int(ms // 3600000); m = int((ms % 3600000) // 60000); s = (ms % 60000) / 1000
                ts = f"{h:02}:{m:02}:{s:06.3f}"
                chapters.append(f"CHAPTER{i:02}={ts}\nCHAPTER{i:02}NAME={title.strip()}")
            
            with open(output_path, 'w', encoding='utf-8') as f:
                f.write("\n".join(chapters))
            os.remove('temp_meta.txt')
    ...
    def convert_to_webvtt(self, srt_path, vtt_path):
        try:
            ...
            with open(vtt_path, 'w', encoding='utf-8') as f:
                f.write("WEBVTT\n\n")
                for line in lines: f.write(line.replace(',', '.'))
            return True
        except: return False
    ...
    def run(self):
        ...
        try:
            ...
            for i, sub in enumerate([s for s in info.get('streams', []) if s.get('codec_type') == 'subtitle']):
                lang = sub.get('tags', {}).get('language', 'und')
                temp_srt = os.path.join(temp_dir, f"int_{i}.srt")
                subprocess.run([ffmpeg, '-y', '-i', self.input_file, '-map', f"0:{sub['index']}", temp_srt], capture_output=True)
                final_sub = temp_srt
                if self.output_format == "mp4_vtt":
                    temp_vtt = temp_srt.replace('.srt', '.vtt')
                    self.convert_to_webvtt(temp_srt, temp_vtt); final_sub = temp_vtt
        ...
        if self.output_format == "mp4_vtt":
            # 1. Video ve Sesi Saf MP4 olarak ayıkla (Metadata temiz, chapters taşınmıyor)
            temp_mp4 = os.path.join(temp_dir, "pure.mp4")
            subprocess.run([ffmpeg, '-y', '-i', self.input_file, '-map', '0:v:0', '-map', '0:a?', 
                           '-c', 'copy', '-tag:v', 'hvc1', '-map_metadata', '-1', '-sn', temp_mp4], capture_output=True)
            
            # 2. Bölümleri OGM olarak çıkart
            chap_path = os.path.join(temp_dir, "chaps.txt")
            has_chaps = self.extract_chapters_ogm(self.input_file, chap_path)

            # 3. MP4Box ile "Tereyağı" birleştirme
            # -brand mp42:isom ve -inter 500 (Apple Standardı)
            box_cmd = [mp4box, "-brand", "mp42:isom", "-ab", "mp42", "-new", "-tight", "-inter", "500"]
            box_cmd.extend(["-add", f"{temp_mp4}#video", "-add", f"{temp_mp4}#audio"])
            
            # Bölümleri ekle
            if has_chaps:
                box_cmd.extend(["-chap", chap_path])
            
            # Altyazıları ekle (GPAC damgasını silmek için name=" ")
            for i, c in enumerate(cleaned_list):
                disable = ":disable" if i > 0 else ""
                box_cmd.extend(["-add", f"{c['path']}:lang={c['lang']}:group=2:name= :tight{disable}"])
            
            box_cmd.extend(["-ipod", output_file])
            subprocess.run(box_cmd, capture_output=True)
        else:
            # MKV Modu (Chapter Title korumalı)
            cmd = [ffmpeg, '-y', '-i', self.input_file]
            for c in cleaned_list: cmd.extend(['-i', c['path']])
            cmd.extend(['-map', '0:v:0', '-map', '0:a?'])
            for i, _ in enumerate(cleaned_list): cmd.extend(['-map', str(i+1)])
            
            # Metadata temizliği: Global ve Stream metadatayı sil, Chapters'a dokunma
            cmd.extend(['-c', 'copy', '-map_metadata:g', '-1', '-map_metadata:s', '-1', '-map_chapters', '0', output_file])
            subprocess.run(cmd, capture_output=True)

        if os.path.exists(temp_dir): shutil.rmtree(temp_dir, ignore_errors=True)
        self.finished_signal.emit(self)

# GUI sınıfları (MainWindow, FileWidget vb.) orijinal yapı korunarak devam eder...
# (Kodun geri kalanı FileWidget ve MainWindow sınıflarını içerir)
...
class MainWindow(QMainWindow):
    ...
    def show_about(self):
        QMessageBox.information(self, "About Fusion", "Fusion v0.3.0\n- Pure Apple Interleave (500ms)\n- Chapter Restoration (OGM Engine)\n- Metadata Purge (Global Only).")
