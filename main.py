...
class ConversionThread(QThread):
    ...
    def clean_subtitle_text(self, text):
        """
        İtalikleri korur, geri kalan her şeyi (ASS, renk, stil) temizler.
        """
        # 1. Mevcut tüm italik varyasyonlarını güvenli bir etikete al
        text = re.sub(r'\{\\i1\}|\\i1|<i>|<I>', '[[I_S]]', text)
        text = re.sub(r'\{\\i0\}|\\i0|</i>|</I>', '[[I_E]]', text)
        
        # 2. Tüm süslü parantezli ASS stil/renk kodlarını ({...}) temizle
        text = re.sub(r'\{[^\}]*\}', '', text)
        
        # 3. Kalan tüm HTML etiketlerini sil
        text = re.sub(r'<[^>]*>', '', text)
    ...
    def process_file_cleaning(self, file_path):
        try:
            ...
            cleaned_lines = []
            for line in lines:
                # SRT yapısını (Sıra no ve Zaman kodu) koru, sadece metni temizle
    ...
    def run(self):
        ...
        for i, sub in enumerate(internal_subs):
            ...
            # FFmpeg'in müdahalesini engellemek için 'text' veya 'copy' yerine doğrudan çekiyoruz
        ...
        for i, c in enumerate(cleaned_list):
            ...
            # Stream bazlı metadata (Dil ve başlık temizliği)
            cmd.extend([f"-metadata:s:s:{i}", f"language={l_code}", f"-metadata:s:s:{i}", "title="])

        # KRİTİK AYARLAR:
        # -map_metadata -1: Global metadatayı (Sarı) siler.
        # -map_chapters 0: Bölümleri (Chapters) korur.
        # -c:s srt: Tüm altyazıları temiz SRT olarak paketler.
...
if __name__ == "__main__":
    app = QApplication(sys.argv); app.setStyle("macos"); w = MainWindow(); w.show(); sys.exit(app.exec())        # 5. Gereksiz karakter temizliği
