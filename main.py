...
class ConversionThread(QThread):
    ...
    def convert_to_webvtt(self, srt_path, vtt_path):
        """QuickTime uyumlu WebVTT: Milisaniyeler nokta ile ayrılmalı."""
        try:
            ...
            with open(vtt_path, 'w', encoding='utf-8') as f:
                f.write("WEBVTT\n\n")
                for line in lines:
                    # SRT'deki virgülleri (00:00:01,500) nokta yap (00:00:01.500)
    ...
    def run(self):
        ...
        if self.output_format == "mp4_vtt":
            ...
            # Adım 1: Sadece video ve sesi al, hvc1 etiketi bas, metadata SİL, chapter KORU.
            ...
            # Adım 2: MP4Box ile paketleme. 
            # -ipod bayrağını en başa alıyoruz. -brand mp42:isom ekliyoruz.
            # Altyazıları :fmt=wvtt yerine :layout=top (opsiyonel) veya direkt ekleyerek deniyoruz.
            box_cmd = [mp4box, "-ipod", "-brand", "mp42:isom", "-add", temp_mp4]
            for c in cleaned_list:
                # Apple için 'wvtt' handler'ı en güvenli yoldur.
                box_cmd.extend(["-add", f"{c['path']}:lang={c['lang']}:fmt=wvtt"])
            
            # Optimize (hinting) işlemini QT hatasını önlemek için sona ekliyoruz
...
# (GUI ve MainWindow sınıfları v0.1.3 ile aynıdır, açılma hatası giderilmiş halidir)
# ... [Buraya önceki GUI kodlarını ekleyebilirsin, ConversionThread dışı aynıdır]
