...
class ConversionThread(QThread):
    ...
    def run(self):
        ...
        # 1. ANALİZ: FFprobe ile dilleri JSON olarak çek
        ...
        # TEMİZLİK: Judas, Title gibi çöpleri siler
        cmd.extend(['-map_metadata', '-1', '-map_chapters', '0'])

        # 4. METADATA: Dilleri geri ekle
...
class FileWidget(QFrame):
    def __init__(self, filename):
        ...
        self.info_icon = QLabel("ⓘ"); self.info_icon.setStyleSheet("color: #999; font-size: 16px;")
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Queue"); self.resize(650, 550); self.setAcceptDrops(True)
        main_layout = QVBoxLayout(); main_layout.setContentsMargins(0, 0, 0, 0); main_layout.setSpacing(0)
        toolbar = QWidget(); toolbar.setFixedHeight(120); toolbar.setStyleSheet("background: white; border-bottom: 1px solid #C0C0C0;")
        t_layout = QHBoxLayout(toolbar); t_layout.setContentsMargins(30, 10, 30, 10); t_layout.setSpacing(45)
        ...
        footer = QWidget(); footer.setFixedHeight(55); footer.setStyleSheet("background: #F8F8F8; border-top: 1px solid #CCC;")
        ...
        self.progress_bar.setFixedWidth(240); self.progress_bar.setFixedHeight(10); self.progress_bar.setTextVisible(False)
        self.progress_bar.setStyleSheet("QProgressBar { background: #E0E0E0; border-radius: 5px; } QProgressBar::chunk { background: #007AFF; border-radius: 5px; }")
    ...
    def create_nav_btn(self, text, icon_char):
        btn = QPushButton(); btn.setFixedSize(100, 100); btn.setText(f"{icon_char}\n\n{text}")
        btn.setStyleSheet("QPushButton { border: none; font-size: 14px; color: #333; background: transparent; font-weight: 600; } QPushButton:hover { background-color: #F0F0F0; border-radius: 20px; }")
        font = QFont(); font.setPointSize(42); btn.setFont(font)
        return btn
...
if __name__ == "__main__":
    app = QApplication(sys.argv); app.setStyle("macintosh"); window = MainWindow(); window.show(); sys.exit(app.exec())        ext_subs = [s for s in subs if s.lower().endswith(('.srt', '.ass')) and s != self.input_file]
        ...
        # Dış altyazı dilini sona ekle (Resim 4'teki hatayı çözer)
        start_idx = len(source_langs['subtitle'])
        for i, sub_path in enumerate(ext_subs):
            match = re.search(r'\.([a-z]{2,3})\.(?:srt|ass)$', sub_path.lower())
            lang = match.group(1) if match else 'und'
