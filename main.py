...
class ConversionThread(QThread):
    ...
    def clean_srt_text(self, text):
        """Metin içindeki italik hariç tüm stil ve etiketleri (bold, color, ass style) temizler."""
        ...
        # 3. Süslü parantez stil kodlarını temizle ({b1}, {c&H...}, {pos...})
        text = re.sub(r'\{[^\}]*\}', '', text)
        
        # 4. İtalikleri standart formatta geri yükle
        text = text.replace('[[i]]', '<i>').replace('[[/i]]', '</i>')
        
        # 5. Gereksiz boş satırları temizle
        return text.strip()

    def process_file_cleaning(self, file_path):
        """Fiziksel dosyayı okur, temizler ve UTF-8 olarak kaydeder."""
        try:
    ...
    def run(self):
        ...
        # 4. Final Muxing (Sıfır Stil - Temiz Altyazılar)
        ...
        # Altyazıları standart srt formatında (temizlenmiş olarak) göm
...
class SublerListWidget(QWidget):
    ...
    def show_context_menu(self, pos):
        ...
        rem_comp = QAction("Remove completed", self)
...
if __name__ == "__main__":
    app = QApplication(sys.argv); app.setStyle("macos"); w = MainWindow(); w.show(); sys.exit(app.exec())        try:
            if not os.path.exists(file_path): return
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                content = f.read()
    ...
    def run(self):
        ...
        # 2. Dahili Altyazıları Çıkar ve Temizle
        cleaned_list = []
        for i, sub in enumerate(internal_subs):
            temp_sub = os.path.join(temp_dir, f"int_{i}.srt")
            subprocess.run([ffmpeg, '-y', '-i', self.input_file, '-map', f"0:{sub['index']}", temp_sub], 
...
class MainWindow(QMainWindow):
    ...
    def setup_menu(self):
        menubar = self.menuBar()
        app_menu = menubar.addMenu("Fusion")
        app_menu.addAction("About", self.show_about)
