...
class ConversionThread(QThread):
    ...
    def run(self):
        ...
        # MUXING
        if self.output_format == "mp4_vtt":
            temp_mp4 = os.path.join(temp_dir, "temp_hvc1.mp4")
            # KRİTİK: -tag:v hvc1 parametresi ile Apple uyumluluğu sağlanır.
            # Metadata temizlenir (-map_metadata -1), Chapter korunur (-map_chapters 0).
            ...
            # MP4Box ile altyazı paketleme
            # -brand mp42 ve -vtt-native bayrakları QuickTime için en güvenli yoldur.
            box_cmd = [mp4box, "-brand", "mp42", "-add", temp_mp4]
            for c in cleaned_list:
                # fmt=wvtt yerine native webvtt handler kullanımı (Apple standartı)
...
# (GUI Bileşenleri ve MainWindow sınıfları bir önceki sürümle aynıdır)
# ...
