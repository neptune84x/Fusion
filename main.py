...
class ConversionThread(QThread):
    ...
    def clean_subtitle_text(self, text):
        """
        Gelişmiş Manuel İtalik Koruma ve Temizlik Sistemi.
        """
        # 1. ASS/SSA italik etiketlerini doğrudan SRT <i> etiketine çevir
        ...
        # 5. Gereksiz HTML etiketlerini temizle (Sadece <i> ve </i> kalsın)
    ...
    def run(self):
        ...
        try:
            # macOS Paket Gizleme Özelliği Korundu
        ...
        for i, sub in enumerate(internal_subs):
            ...
            # KRİTİK: FFmpeg'in SRT çeviricisini baypas etmek için ham 'ass' veya 'copy' alıyoruz
            # Ardından kendimiz temizleyip SRT olarak kaydediyoruz.
            subprocess.run([ffmpeg, '-y', '-i', self.input_file, '-map', f"0:{sub['index']}", "-c:s", "copy", temp_sub_path + ".tmp"], 
                           stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            
            # Geçici dosyayı oku ve temizleyerek nihai srt'ye yaz
            tmp_p = temp_sub_path + ".tmp"
            if os.path.exists(tmp_p):
                with open(tmp_p, 'r', encoding='utf-8-sig', errors='ignore') as f:
                    content = f.read()
                cleaned = self.clean_subtitle_text(content)
                # SRT yapısını korumak için FFmpeg ile bir kez daha ama sadece etiket temizliği bittikten sonra çeviriyoruz
                with open(temp_sub_path, 'w', encoding='utf-8') as f: f.write(cleaned)
                
                # Şimdi güvenli bir şekilde SRT formatına zorla (etiketler artık bizim kontrolümüzde)
                subprocess.run([ffmpeg, '-y', '-i', temp_sub_path, "-c:s", "srt", temp_sub_path + "_final.srt"],
                               stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                
                final_p = temp_sub_path + "_final.srt"
                if os.path.exists(final_p):
                    shutil.move(final_p, temp_sub_path)
        ...
        # MUXING: Chapters ve Metadata Koruma (Kesinlikle Korundu)
