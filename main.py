...
class ConversionThread(QThread):
    ...
    def clean_subtitle_text(self, text):
        """Bold, Color ve ASS stillerini tamamen kazır, sadece italikleri bırakır."""
        # 1. İtalikleri koruma altına al
        text = re.sub(r'<(i|I)>', '[[i]]', text)
        text = re.sub(r'</(i|I)>', '[[/i]]', text)
        
        # 2. Tüm HTML etiketlerini sil (<b>, <font>, <u> vb.)
        text = re.sub(r'<[^>]*>', '', text)
        
        # 3. ASS/SSA stil kodlarını ve süslü parantezli tüm yapıları sil ({...})
        # Bu, bold kalıntılarının en çok gizlendiği yerdir.
        text = re.sub(r'\{[^\}]*\}', '', text)
        
        # 4. SRT içindeki Style satırlarını veya kalınlığı belirten özel karakterleri temizle
        lines = []
        for line in text.splitlines():
            # Eğer satır bir rakam veya zaman kodu değilse, içindeki özel formatları zorla temizle
            if not re.match(r'(\d+:\d+:\d+)|(^\d+$)', line):
                # Çift yıldız (**) veya kalınlık veren Markdown benzeri karakterleri de temizle
                line = line.replace('**', '').replace('__', '')
            lines.append(line)
        
        text = "\n".join(lines)
        
        # 5. İtalikleri geri yükle
        text = text.replace('[[i]]', '<i>').replace('[[/i]]', '</i>')
        return text.strip()

    def process_file_cleaning(self, file_path):
        """Dosyayı okur, her türlü stil kalıntısını temizler ve UTF-8 olarak yazar."""
        try:
    ...
    def run(self):
        ...
        # Temp klasörü yerine gizli .fusiontemp uzantılı geçici dosya yapısı
        temp_data_file = base_path + ".fusiontemp"
        os.makedirs(temp_data_file, exist_ok=True)
        ...
        # 2. Dahili Altyazıları Sök ve Temizle
        cleaned_list = []
        for i, sub in enumerate(internal_subs):
            sub_name = "int_" + str(i) + ".srt"
            temp_sub_path = os.path.join(temp_data_file, sub_name)
            subprocess.run([ffmpeg, '-y', '-i', self.input_file, '-map', "0:" + str(sub['index']), temp_sub_path], 
        ...
        # 4. Muxing & Metadata (Chapters/Dil Korunur, Diğerleri Silinir)
        ...
        # Boldları temiz tutmak için altyazı codecini srt'ye zorla
        cmd.extend(['-c:v', 'copy', '-c:a', 'copy', '-c:s', 'srt', 
                    '-map_metadata', '-1', '-map_chapters', '0', '-y', output_file])
        
        subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        
        # İşlem bitince .fusiontemp dosyasını/klasörünü tamamen sil
        shutil.rmtree(temp_data_file, ignore_errors=True)
...
class SublerListWidget(QWidget):
    ...
    def show_context_menu(self, pos):
        menu = QMenu(self)
        if any(i.is_selected for i in self.items):
            act = QAction("Remove selected", self)
            act.triggered.connect(self.main_window.remove_selected)
            menu.addAction(act)
