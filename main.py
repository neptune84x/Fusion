...
class ConversionThread(QThread):
    ...
    def clean_subtitle_text(self, text):
        """
        İtalikleri korur (<i> ve </i> formatında), geri kalan tüm stil/kod kalabalığını siler.
        """
        if not text: return ""
        
        # 1. Tüm olası italik formatlarını (ASS, HTML, Slash) standart geçici etikete al
        text = re.sub(r'\{\\i1\}|\\i1|<i>|<I>', '[[I_START]]', text)
        text = re.sub(r'\{\\i0\}|\\i0|</i>|</I>', '[[I_END]]', text)
        
        # 2. Tüm süslü parantezli ASS/SSA kodlarını ({...}) tamamen temizle
        text = re.sub(r'\{[^\}]*\}', '', text)
        
        # 3. Kalan diğer tüm HTML etiketlerini (italik olmayan) temizle
        text = re.sub(r'<(?!/?(i|I))[^>]*>', '', text)
        
        # 4. Geçici etiketleri standart SRT italik etiketine çevir
        text = text.replace('[[I_START]]', '<i>').replace('[[I_END]]', '</i>')
        
        # 5. Gereksiz çift sembol temizliği
    ...
    def process_file_cleaning(self, file_path):
        try:
            # BOM'lu veya BOM'suz UTF-8'i güvenle oku
            with open(file_path, 'r', encoding='utf-8-sig', errors='ignore') as f:
                lines = f.readlines()
            
            cleaned_lines = []
            for line in lines:
                # SRT yapısını (Zaman kodu ve sıra no) bozmamak için sadece metin satırlarını seç
    ...
    def run(self):
        ...
        for i, sub in enumerate(internal_subs):
            ...
            # FFmpeg'in otomatik temizlemesini engellemek için doğrudan SRT olarak ayıklıyoruz
        ...
        # KRİTİK AYARLAR: Metadata Silme (-1) ve Chapters Koruma (0)
...
class SublerListWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if not self.selection_rect.isNull():
            painter.setPen(QColor(0, 122, 255, 150)); painter.setBrush(QColor(0, 122, 255, 50))
    ...
    def mouseReleaseEvent(self, event):
        self.selection_start = None; self.selection_rect = QRect(); self.update()

    def show_context_menu(self, pos):
        menu = QMenu(self)
        act_rem = QAction("Remove selected", self)
        act_rem.setEnabled(any(i.is_selected for i in self.items))
        act_rem.triggered.connect(self.main_window.remove_selected)
        act_clear = QAction("Clear completed items", self)
        act_clear.setEnabled(any(i.status == "done" for i in self.items))
        act_clear.triggered.connect(self.main_window.remove_completed)
        menu.addAction(act_rem); menu.addAction(act_clear)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setup_menu(); self.add_btn.clicked.connect(self.open_files)
...
if __name__ == "__main__":
    app = QApplication(sys.argv); app.setStyle("macos"); w = MainWindow(); w.show(); sys.exit(app.exec())        # 5. Gereksiz karakter temizliği ve boşluk düzeltme
        text = text.replace('**', '').replace('__', '')
        return text.strip()
    ...
    def process_file_cleaning(self, file_path):
        try:
            ...
            new_lines = []
            for line in lines:
                # SRT yapısını koru: Zaman koduna veya sıra numarasına dokunma
                if "-->" not in line and not line.strip().isdigit():
                    cleaned = self.clean_subtitle_text(line)
                    if cleaned:
                        new_lines.append(cleaned)
                    else:
                        new_lines.append("") # Boş satırı koru
                else:
    ...
    def run(self):
        ...
        for i, sub in enumerate(internal_subs):
            temp_sub_path = os.path.join(temp_dir_path, f"int_{i}.srt")
            # FFmpeg müdahalesini engellemek için doğrudan SRT olarak çekiyoruz
            subprocess.run([ffmpeg, '-y', '-i', self.input_file, '-map', f"0:{sub['index']}", temp_sub_path], 
                           stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            self.process_file_cleaning(temp_sub_path)
            cleaned_list.append({'path': temp_sub_path, 'lang': sub['lang']})
        ...
        # KRİTİK AYARLAR: Metadata Temizle (-1), Bölümleri Koru (0)
        cmd.extend(['-c:v', 'copy', '-c:a', 'copy', '-c:s', 'srt', 
                    '-map_metadata', '-1', '-map_chapters', '0', output_file])
...
# --- GUI BİLEŞENLERİ (DEĞİŞMEDİ, TAMAMEN KORUNDU) ---
